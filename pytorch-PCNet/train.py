...
def main(args):
    ...
    if args.weights != "":
        if os.path.exists(args.weights):
            ...
            #                      k in model_dict.keys() and k not in ['head.classifier.weight', 'head.classifier.bias']}
    ...
    if args.freeze_layers:
        for name, para in model.named_parameters():
            # 除head外，其他权重全部冻结
            # if "head" not in name and "ca" not in name:
            if "head" not in name:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data-path', type=str,
                        default="G:\deep-learning-for-image-processing-master\data_set\pe")
